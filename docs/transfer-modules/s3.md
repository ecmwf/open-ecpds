...
```properties
s3.region = "eu-west-1"          # AWS region; omit or set to "auto" for auto-discovery
...
!!! tip "Region is optional — or use `s3.region = \"auto\"`"
    If `s3.region` is omitted **or set to `"auto"`**, the module automatically discovers the
    bucket's region at connect time using `GetBucketLocation`. The discovered region is:

    - Logged at `INFO` level: `Auto-discovered region 'eu-west-2' for bucket 'my-bucket'`
    - Appended to the transfer completion comment visible in the Monitoring UI:
      `Sent from DataMover=… to … (rate) [s3.region=eu-west-2]`

    Once you see the discovered region, replace `"auto"` with the real value
    (e.g. `s3.region = "eu-west-2"`) to skip the discovery call on future connections.
    Discovery results are cached in memory (per JVM) so only the first connection to each bucket incurs the extra `GetBucketLocation` call.
    If discovery fails, `us-east-1` is used as a fallback (with a `WARN` log).
...
!!! info "crossRegionAccess always discovers the region upfront"
    When `s3.crossRegionAccess = "yes"` is set, the module **always** calls `GetBucketLocation`
    at connect time — even if `s3.region` is explicitly configured. This prevents stream-replay
    errors that occur when the SDK receives a `301 PermanentRedirect` for a non-resettable upload
    stream and tries to replay it to the correct region.

    If the configured `s3.region` does not match the bucket's actual region, a `WARN` is logged:
    ```
    WARN s3.region 'us-east-2' differs from actual bucket region 'eu-west-2' for bucket 'my-bucket';
         using discovered region to avoid 301 stream-replay errors
    ```
    The transfer comment in the Monitoring UI will also include `[s3.region=eu-west-2]`.

!!! warning "GetBucketLocation call blocked by region-locking SCPs"
    The `GetBucketLocation` discovery call issued when `s3.crossRegionAccess = "yes"` is made
    using `s3.region` if it is already set (falling back to `us-east-1` only when no region is
    known yet, e.g. the very first connection to a new bucket). If your AWS account/organization
    restricts allowed regions via a Service Control Policy (`aws:RequestedRegion`) and `us-east-1`
    is not in the whitelist, that initial-discovery call can be denied — set `s3.region` to the
    bucket's actual (whitelisted) region explicitly to avoid ever needing the `us-east-1` fallback.
    If you don't need cross-region redirect handling, disabling `s3.crossRegionAccess` also avoids
    this call entirely.

!!! tip "PermanentRedirect error? Look for the hint"
    If a transfer fails with `HTTPStatus=301, AWSCode=PermanentRedirect`, the error message now
    includes a plain-English hint:
    ```
    [Hint: bucket is in a different region than s3.region; set s3.crossRegionAccess=yes or correct s3.region]
    ```
    Use `s3.region = "auto"` or `s3.crossRegionAccess = "yes"` to resolve it.
...
s3.stsRegion = ""                      # AWS region for the AssumeRole call (default: global endpoint)
```

!!! info
    By default the STS `AssumeRole` call uses the true global endpoint (`sts.amazonaws.com`),
    independently of `s3.region` (which only applies to the S3 client itself). The base
    credentials (Login / Password) must have `sts:AssumeRole` permission on the target role ARN.

!!! warning "Cross-account AssumeRole denied despite correct permissions? Check region-locking SCPs"
    If `AssumeRole` fails with `is not authorized to perform: sts:AssumeRole` even though the
    same call succeeds via the AWS CLI, the target AWS account/organization likely has a Service
    Control Policy restricting allowed regions (`aws:RequestedRegion`). Such policies commonly
    exempt the **global** STS endpoint but still apply to **regional** STS endpoints (e.g.
    `sts.eu-west-2.amazonaws.com`). OpenECPDS avoids this by always calling the global STS
    endpoint by default — do **not** set `s3.stsRegion` unless you specifically need a regional
    STS endpoint, since doing so re-exposes the module to this restriction.
...
!!! tip "Memory Estimator"
    Rather than computing these numbers by hand, open the host's edit page and click the
    **Memory Estimator** button in the *Options* panel (S3 and Azure hosts). It reads the `s3.*`
    properties currently in the editor (including unsaved edits) and shows the estimated peak
    heap per transfer — and for a configurable number of concurrent transfers — across a range
    of file sizes (10 MB to 100 GB, plus unknown/streaming size), following the same logic
    described in this section (`useByteArrayInputStream`, `multipartSize`, `singlepartSize`,
    `partSize`, `numUploadThreads`, `queueCapacity`, and `disableChunkedEncoding`).
