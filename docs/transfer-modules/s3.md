...
!!! tip "Tuning upload throughput"
    ...
    Memory overhead is at most `numUploadThreads × partSize` MB (e.g. 4 × 25 = 100 MB) of
    additional heap for the in-flight part buffers.
...
s3.multipartSize = "100MB"             # use multipart for files over 100 MB
s3.partSize = "25"                     # 25 MB per part
s3.numUploadThreads = "4"              # 4 threads in-flight → ~100 MB pipelined to S3
s3.sslValidation = "yes"
