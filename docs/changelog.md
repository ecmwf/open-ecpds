...
- **Configurable Product Status Messages**: The "Products Delay" and "Products Resumed" email templates pre-filled on the Product Status page are no longer hardcoded in the JSP. A new *Admin Tasks → Product Status Messages* page lets administrators customize both messages, stored in the `SYS_CONFIG` table; leaving a field empty or unchanged from the built-in default reverts to that default. Both messages support `{{PRODUCT}}` and `{{CYCLE}}` placeholders, automatically replaced with the product name and cycle being viewed (e.g. `GENFO` and `06` on `/do/monitoring/summary/GENFO/06`). The admin page also warns before navigating away with unsaved changes, with a per-field visual indicator.
- **Product Descriptions**: New *Product Descriptions* page lets administrators add, edit and remove a short free-text description for any product (e.g. `GOPER`), stored in the `SYS_CONFIG` table (group `ProductDescription`) alongside the Product Status Messages. Configured descriptions are exposed as a new `{{DESCRIPTION}}` placeholder inside the "Products Delay"/"Products Resumed" messages (substituted with an empty string for products without a configured description).
