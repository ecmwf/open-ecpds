...
## open-ecpds 8.2.0-23092026

- **System Messages: system-wide warning/maintenance banners**: New *System Messages* admin page (`/do/admin/systemmessages`) lets administrators define time-bounded warning/maintenance banners, automatically shown to every user — on the Monitor UI landing page (`/do/start`) and on the Data Portal — from the moment it is created (so users are warned in advance of the announced start time) until the end of the message's configured start/end window, and automatically hidden once it expires, with no manual cleanup required. Each message has a free-text body, a severity level (info/warning/danger), and Start/End times (with quick-duration shortcuts in the add/edit form). See [System Messages](administration/system-messages.md).
- **REST API: monitoring summary endpoints**: Four new/updated `v1/monitoring/summary*` endpoints let external systems retrieve the same step-status data shown on the monitoring pages as JSON: `GET /v1/monitoring/summary` (every product, every cycle — mirrors `/do/monitoring/summary/`), `GET /v1/monitoring/summary/{product}` (every cycle for one product), and the existing `GET /v1/monitoring/summary/{product}/{time}` and `GET /v1/monitoring/summary/{product}/{time}/{step}/{type}` endpoints, now filterable by optional `product`, `time`, `step`, `type` and `status` query parameters and returning two additional fields (`statusCode`, `statusLevel`). All four are now gated by a new, single `monitoringSummaryList` permission — exposed as a checkbox in the *Service Permissions* card of the API Client editor (`/do/user/api/{clientId}`) — closing a gap where the two pre-existing endpoints previously had no permission check at all. See the [REST API Reference](rest-api.md#monitoring).
- **Monitoring: page showing every product/cycle at once**: `/do/monitoring/summary/` (with no product or cycle in the URL) now lists every cycle for every product in a single table, with a new sortable **Prod.** column identifying which product each row belongs to. A new **All Cycles and Products** button, added next to a separator right after the page-selector's **All** button on `/do/monitoring`, links directly to this view; the per-product shortcut previously labelled "All cycles" was also re-capitalised to **All Cycles** for consistency.

## open-ecpds 8.2.0-17092026

- **Control-channel shared secrets, split into two distinct options**: The two independent HMAC challenge-response mechanisms that share a similar configuration pattern now use distinct `[Security]` options to avoid any confusion between them: `rccSharedSecret` (env var `RCC_SHARED_SECRET`) authenticates the `mover/*`/`master/*` REST/JSON control channel used by Proxy/Continental Data Movers relaying through a regular Data Mover to the Master Server (see [Securing the Mover Control Channel](administration/mover-control-channel-security.md)); `cliSharedSecret` (env var `CLI_SHARED_SECRET`) authenticates the direct socket challenge-response between the `ecpds` CLI binary and the Master Server, now also wired end-to-end via the Master's startup script, `ecmwf.properties` and Docker/Kubernetes deployment files (see [`ecpds` CLI documentation](use-cases/ecpds-cli.md#shared-secret-challenge-response-transport-level-optional)). The two secrets protect unrelated connections and do not need to match. Both remain unset/disabled by default, for backward compatibility.
- **S3/Azure Memory Estimator**: The Host page (both the read-only view and the edit form) now shows a **Memory Estimator** button in the *Options* panel for S3 and Azure hosts, which reads the module's properties currently in the editor (including unsaved edits) and estimates the peak heap used per transfer — and for a configurable number of concurrent transfers — across a range of file sizes. For S3 it mirrors the single-part/byte-array vs. bounded multipart streaming logic used by `AmazonS3Module`, including the effect of `s3.useByteArrayInputStream`, `s3.multipartSize`, `s3.singlepartSize`, `s3.partSize`, `s3.numUploadThreads`, `s3.queueCapacity` and `s3.disableChunkedEncoding`; for Azure it mirrors `AzureModule`'s parallel block-blob upload buffer pool (`azure.numBuffers` × `azure.blockSize`) versus its single-PUT streaming path below `azure.multipartSize`.
...
- **System Topology**: a new page in the Monitoring menu (`/do/monitoring/topology`, next to Live Earth) shows a live diagram of the OpenECPDS deployment — the Master Server, its database, every registered Data Mover, and the Monitor instance you are using — grouped by host machine, with down Data Movers shown greyed out and each component's ports/status available on click. Includes an in-page **ⓘ** explanation and a configurable auto-refresh interval (15s/30s/1m/5m/Off). See [System Topology](monitor-ui/monitoring.md#system-topology).
- **Live ECPDS Earth**: New real-time 3D globe view (`/do/monitoring/globe`) visualising ongoing transfers as animated, direction-aware arcs (arrowheads point from mover to Host for Dissemination, and from Host to mover for Acquisition) from the OpenECPDS server/Proxy Hosts to their destination Hosts, colour-coded by status (active, completed, failed/retransmitting), alongside KPI cards for active transfers, active hosts, current throughput, and bytes transferred over the last 24 hours. A Dissemination/Acquisition/Both toggle filters the arcs, markers, country breakdown table, and all KPI cards (including a direction-aware "Transferred (24h)" total) accordingly; Acquisition monitoring is implemented entirely server-side by reusing the existing Mover→Master `updateDownloadProgress` RMI call, with no new RMI methods required. Locations are resolved server-side via GeoIP (including `[GeoIP]` `forced.*` overrides for internal/private addresses), with country/town name labels toggleable independently, and a persistent on-screen warning — naming the unresolved hostname/IP — if the OpenECPDS server's own location has not been configured yet. Added a new Admin Task to allow configuring the geographic location of the local server.
- **Grouped monitoring pages (all cycles/times on one page)**: A product's generic entry in *Product Descriptions* (Type left blank) can now be marked **"Group all cycles/times into one page"**. When set, the product's pill on `/do/monitoring` merges every cycle/time (e.g. `00`, `06`, `12`, `18`) into a single entry — shown with a layers icon instead of a time prefix (e.g. `⧉ GOPER` instead of one pill per `T-GOPER`), coloured by the worst non-"none" status across all of its cycles, and positioned in the header using the earliest scheduled time among them — and links to a new merged page at `/do/monitoring/summary/GOPER` (no time in the URL) listing every cycle's steps/types together in the same table, instead of `/do/monitoring/summary/GOPER/06` per cycle. Useful for products that generate many cycles per day but only a handful of entries per cycle, where the previous one-pill-per-cycle behaviour cluttered the header. Also made the Tips/Description collapse to a single plain-text entry when every type resolves to the same value (e.g. only a generic entry is configured), instead of repeating it once per type. Added a new [Product Descriptions](administration/product-descriptions.md) documentation page.
- **Product Descriptions & Tips (per product/type)**: New *Product Descriptions* page lets administrators add, edit and remove metadata for a product, optionally scoped to one of its types (e.g. `GOPER`/`AN`, as shown in the "Type" column on `/do/monitoring/summary/GOPER/06`); leaving the type blank applies the entry to all of that product's types. Two fields are stored per entry: a **Description**, exposed as the `{{DESCRIPTION}}` placeholder inside the "Products Delay"/"Products Resumed" messages (rendered as a bullet list across the distinct types shown on the current monitoring page, or as plain text when only one applies; empty if none configured), and a **Tips** text, shown as a new expandable "ⓘ" info card right after the Status field on the product/type monitoring page (e.g. `/do/monitoring/summary/GOPER/06/0/AN`). A **Import from Monitoring** button bulk-creates a blank entry for every product/type pair currently seen across the monitoring interface that is not yet configured, ready to be filled in via Edit.
- **Product exclusion filter**: The product-name filter on `/do/monitoring` (e.g. `?application=*ERA5T*`) now accepts a comma-separated list of patterns, each optionally prefixed with `!`/`-` to exclude rather than include (e.g. `*ERA5T*, !*TEST*`). A new **Products** picker panel next to the filter box always lists every known product (not just the ones currently shown) as a clickable chip (green = shown, struck-through = discarded), so products can be hidden and re-enabled directly from the panel without editing the filter text or reloading the page first.
- **Embedded, version-matched documentation**: The Monitor UI's documentation button now opens a copy of the documentation site built into the running release itself (served locally at `/docs/`), instead of always linking to the latest version on GitHub Pages — so end users always see the documentation that matches the exact OpenECPDS version they are running, avoiding confusion from features described online that are not yet in their release. If no embedded copy is available (e.g. some local development/test setups), the link falls back to the online GitHub Pages documentation.
- **Host/Destination Properties+JavaScript separator parsing fix**: Fixed a bug where, when editing a Host or Destination's Properties and JavaScript, saving could sometimes cause part of the JavaScript to be lost and replaced by a garbled, unreadable entry in the Properties list instead. This could happen depending on how the Properties text was formatted before saving. Saving now always correctly keeps the Properties and JavaScript separate, with no data loss.
...
- **Dissemination scheduler improvements**: Several SQL queries driving the dissemination, acquisition, backup, and replication schedulers were refactored for clarity and significantly improved in performance (index hints, eliminated subqueries, corrected `DAF_FILE_SYSTEM IS NOT NULL` guard, `HOLD` entries sorted last). The `TransferServerProvider` connection-slot assignment was also restructured for better load-balancing under concurrent pressure, and the scheduler now delays transmission rather than failing when no transfer server is currently available.
- **TLS certificate status indicator**: The *TLS Certificates* entry in the Admin Tasks card, the left-hand menu, and the start-page quick-links now changes colour dynamically based on the overall certificate health across all connected Movers and Monitors: 🔴 red (expired certificate), 🟡 yellow (self-signed or expiring within 30 days), default grey (all certificates healthy). The status is cached on the MasterServer for up to one hour and immediately invalidated after any certificate deployment or reload, so all MonitorServer instances see the same up-to-date value.
- **HTTP/2 support**: Both the Data Mover HTTP plugin and the Monitoring UI HTTP plugin now support **HTTP/2 with HTTP/1.1 fallback** via ALPN on HTTPS connections. Clients that advertise `h2` in their TLS handshake are served over HTTP/2; clients without ALPN support fall back to HTTP/1.1 transparently. A new `HttpPlugin.http2Enabled` configuration option (default: `true`) allows reverting to HTTP/1.1-only if needed. The Jetty thread pool idle timeout was also corrected from 120 ms to 60 s, preventing excessive thread churn under load.
- **S3 improvements**: Added `s3.region = "auto"` to auto-discover the bucket region via `GetBucketLocation` (cached per JVM; discovered region shown as `[s3.region=eu-west-2]` in the transfer comment). When `s3.crossRegionAccess = "yes"` is set, the region is now always discovered upfront — even if `s3.region` is explicit — preventing stream-replay errors on `301 PermanentRedirect` responses. A `WARN` is logged when configured and actual regions differ. `PermanentRedirect` error messages now include a plain-English hint. Fixed two AWS Organizations region-restriction (SCP) failures affecting cross-account role assumption: the `sts:AssumeRole` call now defaults to the true global STS endpoint instead of the S3 bucket's region (overridable via the new `s3.stsRegion` option), and the `GetBucketLocation` discovery call triggered by `s3.crossRegionAccess` now uses the configured `s3.region` instead of a hardcoded `us-east-1`.
...
- **Configurable ECtrans checksum algorithm**: `ectrans.createChecksum` no longer always computes MD5 — a new `ectrans.checksumAlgorithm` option (`MD5`, `SHA-1`, `SHA-256`, `CRC32`, `Adler32`) selects the algorithm used for the on-the-fly checksum passed to the transfer module. A new common `ectrans.checksumExt` option controls the extension of the sidecar file written by FTP/FTPS/SFTP for any algorithm (auto-derived otherwise, e.g. `.sha256`); the legacy per-module `md5Ext` option is preserved for backward compatibility but now only applies when the algorithm is `MD5`.
- **Data retrieval diagnostics:** The output of the `-waitfor` option of the ECPDS command now provides early visibility into retrieval problems while a group is still in progress. During the wait loop, the command periodically checks for transfers that have already been attempted at least once and are queued for retry (`DAT_REPLICATE_COUNT > 0`, status `SCHE`). When any are found, their details (host, path, size, and last error message) are printed so operators can start investigating without waiting for the full group to complete. Once the loop exits (all files retrieved or timed out), the command also checks for files that ended in `FAIL` status. These would never appear in the queue and could otherwise result in a silently incomplete result. The command lists all failed files (destination, source path, size, and failure reason), ensuring that the caller receives an explicit error rather than a false success.
...
- Added **multi-architecture Docker image support** (`amd64` and `arm64`). Because the `ecpds-mover` image contains a native shared library (`libsocketoptions.so`) compiled for the host CPU, cross-compilation is not possible in a single build step. The release workflow now supports a two-machine approach: run `make push-native` (or `make push-sa-native` for the standalone image) independently on an x86\_64 and an aarch64 host — the two builds can run in parallel — then run `make manifest` (or `make sa-manifest`) once on either machine to combine the arch-specific images into a single multi-arch manifest in the registry. Single-arch pushes remain available via `make push` / `make push-sa` for scenarios where only one architecture is needed.
- The **`ecpds` command-line client** (compiled from `ECpdsCmd.c`) is now bundled in the standalone container and installed at `/usr/local/bin/ecpds`, making it immediately available for submitting files to the Master Server without any additional setup (`docker exec standalone ecpds …`). A dedicated minimal **CLI Docker image** (`ghcr.io/ecmwf/open-ecpds/cli`) is also published for users who prefer to run the tool via Docker without pulling the full standalone image. Pre-compiled binaries for `amd64` and `arm64` are additionally published as assets on each GitHub Release (`ecpds-amd64` / `ecpds-arm64`) for direct download — the binary requires only OpenSSL at runtime. Use `make release-tools` to produce the correctly named binary for the current host architecture.
- Added **centralised TLS certificate management** from the Administration page of the Monitoring UI. Administrators can view certificate details (subject, issuer, SANs, validity, fingerprint), generate self-signed certificates or CSRs, import PEM / PKCS#12 / JKS certificates, download the public certificate, deploy it to all connected Monitors and Data Movers (covering HTTPS and MQTT/TLS), and hot-reload it from disk on any or all components — without any service interruption. On first startup, each component automatically generates its own unique self-signed certificate if none exists.
- Added a native **SFTP/SCP incoming plugin** for the Data Portal, implemented on top of **Apache MINA SSHD**. Data Portal users can now push and pull files over SFTP and SCP without requiring any commercial API. Both password and public-key (SSH authorised keys) authentication are supported. Updated documentation accordingly.
- Added native **WebDAV support** to the Data Portal as both an incoming and outgoing protocol. Users can now browse, upload, and download files through any WebDAV-compatible client — including Finder on macOS, Windows Explorer, Nautilus on Linux, and command-line tools such as `cadaver` and `curl` — using the portal's HTTPS endpoint. WebDAV is also available as an ECtrans module, supporting both **dissemination** (pushing files from OpenECPDS to a remote WebDAV server) and **acquisition** (retrieving files from a remote WebDAV server into OpenECPDS) as part of automated transfer workflows.
...
- Improved the **Host duplication** workflow with type-aware handling across all six host types. Dissemination, Acquisition, and Proxy hosts now show a destination picker so the duplicate can be immediately associated with a Destination; Replication, Backup, and Source hosts go directly to a standalone copy. The picker description adapts to the host type (outbound transfers, data retrieval, or pre-replication). An optional **Nickname** field is also available in the dialog for all host types, allowing the new host to be given a distinct name at duplication time rather than inheriting the original's nickname.
...
- Added **IncomingUser (data user) authentication** to the `ecpds` command-line client. The new `-user <login>` and `-pass <password>` options let any unprivileged user authenticate against the OpenECPDS user database and submit files without requiring a privileged source port (< 1024). When credentials are supplied, the Master Server validates them via `getIncomingProfile()`, checks that the authenticated user is allowed to access the requested destination, and enforces the path-level permissions defined in the user's properties: **`put`** for a normal submission or requeue, and **`delete`** for a purge (`-purge`). Without `-user`/`-pass` the legacy ECUSER/privileged-port authentication path is unchanged.
- Upgraded **Eclipse Jetty** from 10.0.26 (EOL) to **12.1.12**. Jetty 12 introduces a strict EE split: the monitoring WAR (Struts, `javax.servlet.*`) runs in an EE8 `WebAppContext`; the Data Mover's S3 and REST handlers run in an EE10 `ServletContextHandler` with `jakarta.servlet.*`; WebDAV (Jackrabbit) and the WebSocket AI-assistant endpoint remain in EE8 contexts. Both EE tiers coexist on the same Jetty server instance. The upgrade eliminates several known CVEs in the Jetty 10 line and aligns the project with the current supported Jetty release.
- Removed the **Apache Wink** REST framework (EOL, no longer maintained) and replaced it with **Eclipse Jersey 3.1** across all REST endpoints. Jersey is the JAX-RS 3.1 reference implementation, supports `jakarta.ws.rs.*`, and is actively maintained. All existing REST APIs are preserved unchanged; the migration is entirely internal. The `jackson-all` 1.x dependency was also removed and replaced with the current **Jackson 2.x** (`jackson-databind`, `jackson-core`, `jackson-annotations`).
- Added a full **API Client management UI** under the *Access Control* section of the Monitoring Interface. Administrators can create, view, edit, and delete API clients. A randomly generated secret is shown once at creation time (and on reset) and never stored in plain text — only a SHA-256 hash is retained. **Service Permissions** are managed directly on the client detail page: a guided permission picker lists all available services, with checkboxes for quick selection and a regex pattern editor for fine-grained control. A *Permission Guide* offcanvas explains each service and provides regex examples. A *No API Clients only* filter on the list page makes it easy to identify them. A migration step at startup automatically imports any API client definitions previously held in `ecmwf.properties`.
