...
- **Mover control channel authentication**: The `mover/*` and `master/*` REST/JSON endpoints used by Proxy/Continental Data Movers to relay control-plane calls (health checks, host/transfer updates, ecAuth token requests, live-transfer statistics, ...) through a regular Data Mover to the Master Server are now authenticated with an HMAC-SHA256 request signature, enabled by configuring the existing `[Security] sharedSecret` option identically on every Data Mover and Proxy/Continental Data Mover. Left unset (the default), the channel remains exactly as before, for backward compatibility — see [Securing the Mover Control Channel](administration/mover-control-channel-security.md).
