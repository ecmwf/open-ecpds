...
- **Live ECPDS Earth**: New real-time 3D globe view (`/do/monitoring/globe`) visualising ongoing transfers as animated arcs from the OpenECPDS server to their destination Hosts/Proxy Hosts, colour-coded by status (active, completed, failed/retransmitting), alongside KPI cards for active transfers, active hosts, current throughput, and bytes transferred over the last 24 hours. Locations are resolved server-side via GeoIP (including `[GeoIP]` `forced.*` overrides for internal/private addresses), with country/town name labels toggleable independently, and a persistent on-screen warning — naming the unresolved hostname/IP — if the OpenECPDS server's own location has not been configured yet.
