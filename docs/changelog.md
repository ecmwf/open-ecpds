...
- **`waitForGroup` acquisition diagnostics**: The `waitforgroupReq` command in the ECPDS plugin now provides early visibility of retrieval problems while a group is still in progress. During the wait loop, the command periodically checks for transfers that have already been attempted at least once and are queued for retry (`DAT_REPLICATE_COUNT > 0`, status `SCHE`); when any are found, the file details (host, path, size, and last error message) are printed so operators can start investigating without waiting for the full group to complete. Once the loop exits (all files retrieved or timed out), the command also checks for files that ended in `FAIL` status — which would never appear in the queue and would otherwise cause a silent incomplete result — and calls `stopAndError` with the list of up to the first 10 failed files (destination, source path, size, and failure reason) so the caller receives an explicit error rather than a false success.
