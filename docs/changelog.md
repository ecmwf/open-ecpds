...
- Added **centralised TLS certificate management** accessible from the Administration page of the Monitoring UI. Administrators can view certificate details (subject, issuer, validity, fingerprint), generate self-signed certificates, generate CSRs, import certificates in PEM / PKCS#12 / JKS format, download the public certificate, and deploy the active certificate to all connected Data Movers — all from a single page and without any service interruption (certificates are hot-reloaded via `SslContextFactory.reload()`). On first startup, a unique self-signed certificate is automatically generated for each component if none exists. A subtle TLS notice banner is shown to administrators when a self-signed certificate is in use, linking directly to the certificate management page.
