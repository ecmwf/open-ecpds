...
- **GCS parallel composite upload**: The GCS transfer module can now optionally split large uploads into parts uploaded concurrently as temporary objects, then server-side composed into the final object (Google's "parallel composite upload"). New `gcs.parallelUpload` (off by default), `gcs.parallelUploadNumThreads` and `gcs.parallelUploadPartSize` options; existing behaviour is unchanged unless explicitly enabled.
