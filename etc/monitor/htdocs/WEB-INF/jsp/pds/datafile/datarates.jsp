...
<c:if test="${not empty trafficError}">
  <div class="alert alert-danger d-flex align-items-center gap-2 mt-3">
    ...
    <span>Failed to load traffic data: <strong><c:out value="${trafficError}"/></strong></span>
  </div>
</c:if>
...
<c:if test="${not empty trafficList}">
...
<%-- Embed raw server-side data as JS arrays --%>
<script>
const _tLabels = [], _tBytes = [], _tDuration = [], _tFiles = [];
<c:forEach var="t" items="${trafficList}">
...
<script>
// -- Formatting helpers -------------------------------------------------------
function computeRate(bytes, duration) {
  if (duration <= 0) return 0;
  var bps = bytes / (duration / 1000.0);
  var mbits = bps * 8.0 / 1048576.0;
  return Math.round(mbits * 100) / 100;
}
...
function getFilteredTableRows() {
  var allIdx = _sortedIdx || tLabels.map(function(_,i){return i;});
...
function buildTable() {
  ...
  pageRows.forEach(function(i) {
    html +=
      '<tr>' +
      '<td>' + tLabels[i] + '</td>' +
      '<td title="' + tBytes[i] + ' bytes">' + fmtBytes(tBytes[i]) + '</td>' +
      '<td>' + fmtDur(tDuration[i]) + '</td>' +
      '<td class="' + rateClass(tRates[i]) + '">' + tRates[i].toFixed(2) + '</td>' +
          ...
          callbacks: {
            label: function(ctx) {
              ...
              return ctx.dataset.label.startsWith('Volume')
                ? '  Volume: ' + fmtBytes(d.bytes[i])
                : '  Rate: '   + d.rates[i].toFixed(2) + ' Mbit/s';
            }
          }
        }
      },
