...
## Aggregate statistics FIRST, before any deletions, to avoid missing rows
## that may be deleted further down in this script.

## Compute the bandwidth per destination (skip any destination already recorded to avoid duplicates)
INSERT INTO BANDWIDTH SELECT
  dt.DES_NAME,
  DATE_FORMAT(FROM_UNIXTIME(dt.DAT_START_TIME/1000),'%Y-%m-%d') AS BAN_DATE,
  SUM(dt.DAT_SENT) AS BAN_BYTES,
  SUM(dt.DAT_DURATION) AS BAN_DURATION,
  COUNT(*) AS BAN_FILES
FROM
  DATA_TRANSFER dt
WHERE
  FROM_UNIXTIME(dt.DAT_START_TIME/1000,'%Y-%m-%d') = DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d")
  AND dt.STA_CODE = 'DONE'
  AND NOT EXISTS (
    SELECT 1 FROM BANDWIDTH
    WHERE DES_NAME = dt.DES_NAME
    AND BAN_DATE = DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), '%Y-%m-%d')
  )
GROUP BY
  BAN_DATE,
  dt.DES_NAME;

## Aggregate portal traffic into PORTAL_TRAFFIC_DAILY for yesterday (skip if already done)
INSERT INTO PORTAL_TRAFFIC_DAILY
  (PTD_USER, PTD_DATE, PTD_CONNECTIONS, PTD_BYTES_IN, PTD_BYTES_OUT, PTD_DURATION_IN, PTD_DURATION_OUT)
SELECT
  PTR_USER,
  DATE(PTR_TIME),
  SUM(PTR_CONNECTIONS),
  ...
  SUM(PTR_BYTES_OUT),
  SUM(PTR_DURATION_IN),
  SUM(PTR_DURATION_OUT)
FROM PORTAL_TRAFFIC
WHERE
  (SELECT COUNT(*) FROM PORTAL_TRAFFIC_DAILY WHERE PTD_DATE = DATE(NOW()-INTERVAL 1 DAY)) = 0
  AND DATE(PTR_TIME) = DATE(NOW()-INTERVAL 1 DAY)
GROUP BY
  PTR_USER, DATE(PTR_TIME);
...
## Remove PORTAL_TRAFFIC rows older than 7 days (daily aggregates kept in PORTAL_TRAFFIC_DAILY)
LOOP 1000 DELETE FROM PORTAL_TRAFFIC
WHERE PTR_TIME < DATE_FORMAT(DATE(NOW() - INTERVAL 7 DAY), '%Y-%m-%d 00:00:00')
LIMIT 1000;
...
##
## First remove any TRANSFER_STATISTICS rows referencing the same
## DATA_TRANSFER rows (foreign key constraint added with TRANSFER_STATISTICS).
##
