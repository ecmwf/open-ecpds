 ...
 * - Supporting operations such as scheduling, starting, stopping, and checking
 * tasks
 * - Ensuring secure and efficient data transmission with error handling and
 * cleanup mechanisms
 *
 * Usage:
 * The program accepts a variety of command-line options to customize its
 * behavior. For detailed usage instructions, run the program with the -help
 * option.
 *
 ...
 * @param response A pointer to the buffer where the computed HMAC response will
 * be stored.
 */
void compute_response(const char *challenge, unsigned char *response) {
  unsigned int len = RESPONSE_SIZE;
  HMAC(EVP_sha256(), SECRET, strlen(SECRET), (unsigned char *)challenge,
       CHALLENGE_SIZE, response, &len);
}

/**
 * Encodes the given byte array (challenge) in Base64 format and returns it as a
 * string.
 *
 ...
 * @return A pointer to a dynamically allocated string containing the Base64
 * encoded challenge. The caller is responsible for freeing the allocated
 * memory.
 *
 ...
 * to hold the encoded data and ensures that no newlines are added to the
 * output. After encoding, it retrieves the encoded string and returns it.
 */
 ...
 */
void error(char *str) { message("error", str); }
 ...
 */
void warning(char *str) { message("warning", str); }

/**
 * Retrieves the local hostname.
 * This function gets the local hostname and returns it. If an error occurs, it
 * returns "[unknown]".
 *
 ...
 * This function gets the current local time and formats it as "MM/DD/YY
 * HH:MM:SS".
 *
...
/**
 * Attempts to establish a non-blocking connection to a specified address with a
 * timeout. The function sets the socket to non-blocking mode, attempts to
 * connect, and waits for the connection to be established within the given
 * timeout period. If the connection is successful, the socket is reset to its
 * original state. The function also includes optional debugging output.
 *
...
/**
 * Reads a line of text from a socket descriptor, stopping at a newline
 * character. The function reads one character at a time from the socket until a
 * newline character is encountered or the buffer is full. The newline character
 * is replaced with a null terminator to form a proper C string. The function
 * also includes optional debugging output.
 *
...
/**
 * Opens a connection to a specified hostname and port with a timeout and
 * optional retry mechanism. The function resolves the hostname, creates a
 * socket, and attempts to connect to the specified port within the given
 * timeout period. It also sets socket options such as SO_KEEPALIVE and includes
 * optional debugging output. If the connection fails, the function retries the
 * connection a specified number of times with a delay between attempts.
 *
 * @param hostname The hostname to connect to.
 * @param port The port number to connect to, as a string.
 * @param timeout_seconds The timeout duration for each connection attempt, in
 * seconds.
...
int openConnection(char *hostname, char *port, int timeout_seconds) {
  ...
  if (bindresvport(s, &sa) >= 0) {
    ...
    if (debug) {
      fprintf(stderr,
              "[%.19s] DEBUG: socket binded to an unprivileged port "
              "(bindresvport failure)\n",
              getTime());
    }
  }
  ...
  if ((err = connect_wait(s, (struct sockaddr *)&si, sizeof(si), &timeout)) ==
      -1) {
    if (debug) {
  ...
  if (debug) {
    fprintf(stderr,
            "[%.19s] DEBUG: connected(%d) on %s:%s(local=%d) "
            "(SO_KEEPALIVE=%d,TCP_NODELAY=%d)\n",
...
/**
 * Attempts to establish a connection to one of the provided hostnames on the
 * specified port. The function tries to connect to each hostname in a
 * randomized order, with a retry mechanism that includes a delay between
 * attempts. If a connection is successfully established, the socket descriptor
 * is returned. If all attempts fail, the function returns -1. If the connection
 * is successfull, this function receive a challenge from the server, compute
 * and send the response back.
 *
 * @param hostnames A comma-separated string of hostnames to attempt connections
 * to.
 * @param port The port number to connect to, as a string.
 * @param timeout_seconds The timeout duration for each connection attempt, in
 * seconds.
  ...
  for (int i = 0; s == -1 && i < TRY_COUNT; i++) {
    ...
    for (int j = 0; j < numHostnames; j++) {
      ...
      if (s != -1) {
        ...
        if (strlen(SECRET) > 0) {
          if (debug) {
            fprintf(stderr,
                    "[%.19s] DEBUG: challenge-response authentication\n",
                    getTime());
          }
          ...
          while (total_bytes_read < BASE64_ENCODED_SIZE) {
            bytes_read = read(s, encodedChallenge + total_bytes_read,
                              BASE64_ENCODED_SIZE - total_bytes_read);
          ...
          while (total_bytes_written < RESPONSE_SIZE) {
            bytes_written = write(s, response + total_bytes_written,
                                  RESPONSE_SIZE - total_bytes_written);
    ...
    if (i + 1 < TRY_COUNT) {
      if (debug) {
        fprintf(
            stderr,
            "[%.19s] DEBUG: connect failed (%d/%d) - waiting for %d seconds\n",
 ...
 * The function formats the key and value into a single message, checks for
 * potential buffer overflow, and writes the message to the socket. It includes
 * error handling and optional debugging output.
 *
 * @param sd The socket file descriptor.
 ...
 * This function is a wrapper around sendCommand, used for sending commands that
 ...
 * @return 0 on success, or -1 on error.
 */
int sendAction(int sd, char *key) { return sendCommand(sd, key, ""); }

/**
 * Sends a boolean value to a server via a socket descriptor.
 * This function sends the string "true" if the condition is true, otherwise it
 * does nothing.
 *
 * @param sd The socket file descriptor.
 ...
 * This function converts the integer value to a string and sends it using
 * sendCommand.
 *
 * @param sd The socket file descriptor.
 ...
 * This function converts the 64-bit integer value to a string and sends it
 * using sendCommand.
 *
 * @param sd The socket file descriptor.
 ...
 * This function reads a line from the socket, processes the command, and
 * extracts the value if a key is provided. It includes error handling and
...
/**
 * Splits a string at the last occurrence of a separator and returns the next
 * parameter. This function modifies the input string by inserting a null
 * terminator at the position of the last occurrence of the separator,
 ...
 * @return A pointer to the next parameter after the separator, or NULL if the
 ...
 * This function reports a timeout error, removes a temporary file if defined,
 ...
 * This function prints the usage instructions and available command-line
 * options to stdout.
 *
...
int usage(void) {
  ...
  fprintf(stdout, " -lifetime    {arg} - lifetime of the data file (default: "
                  "2d) (*****)\n");
  fprintf(stdout, " -delay       {arg} - transmission delay (default: "
                  "immediate transfer) (*****)\n");
  fprintf(stdout, " -at          {arg} - transmission date (default: immediate "
                  "transfer)\n");
  fprintf(stdout, " -format      {arg} - define the date format (default: "
                  "yyyyMMddHHmmss)\n");
  fprintf(
      stdout,
      " -group       {arg} - define the transfer group (default: random)\n");
  fprintf(
      stdout,
      " -version     {arg} - optional version associated with the DataFile\n");
  fprintf(stdout, " -reqid       {arg} - optional DataFileId for the "
  ...
  fprintf(stdout, " -index             - in groupby mode source file is index "
                  "of source files\n");
  fprintf(stdout, " -noretrieval       - file not retrieved in groupby mode "
                  "(taken from source)\n");
  fprintf(
      stdout,
      " -expected          - the task is identified with the metadata(s)\n");
  fprintf(
      stdout,
      " -started           - the task is identified with the metadata(s)\n");
  fprintf(
      stdout,
      " -completed         - the task is identified with the metadata(s)\n");
  fprintf(
      stdout,
      " -reset             - the task is identified with the metadata(s)\n");
  fprintf(stdout, " -buffer            - the task is identified with the "
                  "metadata(s) (*****)\n");
  fprintf(stdout,
          " -asap              - send file as soon as possible (******)\n");
  fprintf(stdout, " -event             - notification triggered once data is "
                  "available (e.g. mqtt)\n");
  fprintf(stdout, " -standby           - spool the data file only\n");
  fprintf(stdout,
          " -remove            - remove source when transfer successful\n");
  fprintf(stdout, " -requeue           - requeue a dataFile and reset the "
                  "related transfer(s)\n");
  fprintf(
      stdout,
      " -purge             - purge the dataFile and the related transfer(s)\n");
  fprintf(stdout, " -force             - force a requeue when a duplicate "
                  "dataFile is found\n");
  fprintf(stdout,
          " -buffsize          - buffer size for read and write (default: %d "
          "bytes)\n",
          BUFFSIZE);
  fprintf(stdout,
          " -connectTimeoutSec - when connecting to Masters/Movers (default: "
          "%d seconds)\n",
          CONNECT_TIMEOUT_IN_SECONDS);
  fprintf(
      stdout,
      " -tryCount          - when connecting to Masters/Movers (default: %d)\n",
      TRY_COUNT);
  fprintf(stdout,
          " -tryDelaySec       - when connecting to Masters/Movers (default: "
          "%d seconds)\n",
          TRY_DELAY_IN_SECONDS);
  fprintf(stdout, " -verbose           - verbose mode on\n");
  fprintf(stdout, " -start             - (re)start of specified destination\n");
  fprintf(stdout,
          " -stop              - graceful stop of specified destination\n");
  fprintf(stdout, " -opts              - send debug options\n");
  fprintf(stdout, " -help              - this message\n");
  fprintf(stdout, " -v                 - version number\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "     (*) If successful, a DataFileID is returned, which can "
                  "be used to keep track\n");
  fprintf(stdout,
          "         of the transfer requests through the web interface.\n");
  fprintf(stdout, "    (**) Notify the monitoring module that a task is "
                  "expected/started/completed.\n");
  fprintf(stdout, "   (***) Allow starting or stopping the download of the "
                  "preset files on ecpds.\n");
  fprintf(
      stdout,
      "  (****) Wait for a group of preset files to be retrieved on ecpds.\n");
  fprintf(stdout, " (*****) Duration in weeks, days, hours, minutes or seconds "
                  "(e.g. 1w|2d).\n");
  fprintf(stdout, "(******) File sent once all the files from the same group "
                  "are retrieved.\n");
 ...
 * This function initializes necessary variables, sets up signal handlers, and
 * parses command-line options. It handles various options such as setting
 * connection timeouts, specifying source and target files, and configuring
 * other parameters. The function also includes error handling and usage
 * instructions.
 *
...
int main(int argc, char *argv[]) {
  ...
  off64_t filelen = 0;
  off64_t fileres = 0;
  int sd = -1, pd = -1, fd = -1, verb = 0, requeue = 0, force = 0, purge = 0,
      remv = 0, timefile = 0, del = 0, useTmpFile = 0, fsize = 0, res = -1,
      standby = 0, index = 0, expected = 0, started = 0, completed = 0,
      reset = 0, statsrc, tst = -1, local = 1, scheduler = 0, start = 0,
  ...
  while (argc >= 1) {
    if (strcmp(*argv, "-echost") == 0) {
      ...
      fsize <<= (q = tolower(*p)) == 'k' ? 10 : q == 'm' ? 20 : 0;
  ...
  /**
   * The following validate command-line options for compatibility and
   * correctness. Checks for mutually exclusive options and ensures that
   * required options are used together. It provides error messages and usage
   * instructions if any validation fails.
   */
   ...
   * This block handles the case where input is expected from stdin and stores
   * it in a temporary file. It ensures that the -target option is provided when
   * reading from stdin.
   */
  if (source == NULL && waitfor == NULL && !scheduler && !purge && !completed &&
      !started && !expected && !reset) {
   ...
   * This block handles the case where the target is not specified and derives
   * it from the source file name.
   */
  if (target == NULL && waitfor == NULL && !scheduler && !purge && !completed &&
      !expected && !started && !reset) {
   ...
   * This block prints various pieces of information to stderr for debugging and
   ...
   * This block prints more detailed information about the operation if the
   * verbose flag is set and specific conditions are not met.
   */
  if (waitfor == NULL && !scheduler && !completed && !expected && !started &&
      !reset && verb) {
  ...
  if (waitfor == NULL && !scheduler && !purge && !completed && !expected &&
    ...
    if (realpath(source, resolved) != NULL) {
      ...
      warning("getting canonicalized absolute source pathname (realpath)");
    }
    ...
    if (S_ISFIFO(stcert.st_mode)) {
      ...
      if (index) {
        /* The source file is an index file so we have to add the size of all
         * the files specified!
         */
        index = 0;
        filelen = 0;
        {
          ...
          while (fgets(line, sizeof(line), file)) {
            ...
            if (fileName != NULL && strlen(fileName) > 0 &&
              ...
              if (stat64(fileName, &st) < 0) {
                ...
                char buf[2048];
                sprintf(buf, "getting %s status (stat64)", line);
                error(buf);
                fclose(file);
                goto clean;
              }
          ...
          if (!index) {
            ...
            if (index == 1) {
              /* Let's optimise and treat it as a normal ecpds without the index
               * option */
              if (verb)
  ...
  if (completed || expected || started || reset) {
    if ((bufferMon != NULL && sendCommand(sd, "BUFFER", bufferMon) == -1) ||
        sendCommand(sd, "METADATA", metadata) == -1 ||
        (at != NULL && sendCommand(sd, "AT", at) == -1) ||
        sendAction(sd, completed  ? "COMPLETED"
                       : expected ? "EXPECTED"
                       : reset    ? "RESET"
                                  : "STARTED") == -1 ||
        receiveCommand(sd, "MESSAGE", message, 512) == -1)
  ...
  if (scheduler) {
    if (start || stop) {
      if (destination == NULL) {
        if (sendAction(sd, start ? "SCHEDULERSTART" : "SCHEDULERSTOP") == -1 ||
            receiveCommand(sd, "MESSAGE", message, 512) == -1)
          goto clean;
      } else { // This is a start/stop destination request
        if (sendCommand(sd, "DESTINATION", destination) == -1 ||
            sendAction(sd, start ? "DESTINATIONSTART" : "DESTINATIONSTOP") ==
                -1 ||
            receiveCommand(sd, "MESSAGE", message, 512) == -1)
          goto clean;
      }
  ...
  {
    int mspds = 0;
    if (sendBoolean(sd, "TIMECRITICAL", mspds) == -1 ||
        sendCommand(sd, "DESTINATION", destination) == -1 ||
        sendInteger(sd, "TIMEFILE", timefile) == -1 ||
        sendCommand(sd, "FORMAT", format) == -1 ||
        (index > 0 && sendInteger(sd, "INDEX", index) == -1) ||
        sendCommand(sd, "GROUP", group) == -1 ||
        sendCommand(sd, "REQID", reqid) == -1 ||
        sendCommand(sd, "PRIORITY", priority) == -1 ||
        sendCommand(sd, "UNIQUENAME", version) == -1 ||
        sendCommand(sd, "IDENTITY", identity) == -1 ||
        sendCommand(sd, "ORIGINAL", tmp == NULL ? original : tmp) == -1 ||
        sendCommand(sd, "SOURCE", source) == -1 ||
        sendCommand(sd, "TARGET", target) == -1 ||
        sendCommand(sd, "LIFETIME", lifetime) == -1 ||
        sendCommand(sd, "DELAY", delay) == -1 ||
        sendCommand(sd, "AT", at) == -1 ||
        sendCommand(sd, "METADATA", metadata) == -1 ||
        (filelen != -1 && sendLong64(sd, "SIZE", filelen) == -1) ||
        sendCommand(sd, "GROUPBY", groupby) == -1 ||
        sendBoolean(sd, "NORETRIEVAL", noretrieval) == -1 ||
        sendBoolean(sd, "FORCE", force) == -1 ||
        sendBoolean(sd, "REQUEUE", requeue) == -1 ||
        sendBoolean(sd, "STANDBY", standby) == -1 ||
        sendBoolean(sd, "ASAP", asap) == -1 ||
        sendBoolean(sd, "EVENT", event) == -1 ||
        sendBoolean(sd, "REMOVE", remv) == -1 ||
        sendBoolean(sd, "PURGE", purge) == -1 || sendAction(sd, "PUT") == -1 ||
        (!purge && groupby == NULL &&
         receiveCommand(sd, "TARGET", target, MAXPATHLEN) == -1) ||
        (!purge && groupby == NULL &&
         receiveCommand(sd, "ECPROXY", hosts, 512) == -1) ||
        receiveCommand(sd, "MESSAGE", message, 512) == -1) {
      goto clean;
    }
  }
   ...
   * This block iterates through the list of ecproxies, establishes connections,
  ...
  while (hasMore) {
    ...
    if ((pd = tryConnection(ecproxyHost, ecproxyPort,
                            CONNECT_TIMEOUT_IN_SECONDS)) == -1 ||
        sendCommand(pd, buf, "") == -1 || sendCommand(pd, "OPTS", opts) == -1) {
      continue;
    }
   ...
   * This block sends the stats, sends a BYE command, and receives a message
   * from the server.
   */
  if (sendCommand(sd, "HOST", stats) == -1 || sendAction(sd, "BYE") == -1 ||
      receiveCommand(sd, "MESSAGE", message, 512) == -1) {
    goto clean;
  }
 ...
 * This block ensures that all allocated resources are freed and file
