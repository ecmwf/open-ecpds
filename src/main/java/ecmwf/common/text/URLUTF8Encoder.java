...
public final class URLUTF8Encoder {
    ...
    /** The Constant hex. */
    private static final String[] hex = { "%00", "%01", "%02", "%03", "%04", "%05", "%06", "%07", "%08", "%09", "%0a",
    ...
    public static String encode(final String s) {
        final var sbuf = new StringBuilder();
        final var len = s.length();
        for (var i = 0; i < len; i++) {
            final int ch = s.charAt(i);
            if (('A' <= ch && ch <= 'Z') || ('a' <= ch && ch <= 'z')) { // 'A'..'Z'
                sbuf.append((char) ch);
    ...
    public static String unescape(final String s) {
        ...
        for (int i = 0, more = -1; i < l; i++) {
            ...
            b = switch (ch = s.charAt(i)) {
            case '%' -> {
                ch = s.charAt(++i);
                final var hb = (Character.isDigit((char) ch) ? ch - '0' : 10 + Character.toLowerCase((char) ch) - 'a')
                        & 0xF;
                ch = s.charAt(++i);
                final var lb = (Character.isDigit((char) ch) ? ch - '0' : 10 + Character.toLowerCase((char) ch) - 'a')
                        & 0xF;
                yield hb << 4 | lb;
            }
            default -> ch;
