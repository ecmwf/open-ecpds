...
public final class AzureModule extends TransferModule {
	...
	public void connect(final String location, final ECtransSetup setup) throws IOException, URISyntaxException {
		...
		try {
			service = BlobServiceClientCache.getBlobServiceClientCache(getSetup().getString(HOST_AZURE_SAS_URL),
					getSetup().getString(HOST_AZURE_SAS_SUBSCRIPTION_KEY), user, password, url, containerName,
	...
	/**
	 * Gets the MD 5 input stream. Utility to initialize the md5 object for a given
	 * input stream if required from the configuration.
	 *
	 * @param in the in
	 *
	...
	/**
	 * Gets the mark supported input stream. This make sure the input stream support
	 * mark and reset features.
	 *
	 * @param in the in
	 *
	 ...
	 * Gets the sas token.
	 *
	 * @param sasUrl             the sas url
	 * @param sasSubscriptionKey the sas subscription key
	...
	private static final class BlobServiceClientCache {
		 ...
		 * @param url                the url
		 * @param containerName      the container name
		 * @param mkContainer        the mk container
		...
		static BlobServiceClientCache getBlobServiceClientCache(final String sasUrl, final String sasSubscriptionKey,
				final String accountName, final String accountKey, final String url, final String containerName,
				...
				try {
					...
					if (cacheIten == null) {
						...
						if (isNotEmpty(accountName) && isNotEmpty(accountKey)) {
							// The authentication is done through the credentials
							...
							// We are requested to create a sas token
