...
final class BrokerHibernate implements Broker {
	...
	/**
	 * This code is creating an instance of the BrokerHibernate class, which has a
	 * constructor that initializes some fields. It first obtains a SessionFactory
	 * from the HibernateSessionFactory class and uses it to open a new session with
	 * sessionFactory.openSession(). It also gets a reference to the cache
	 * associated with the session using sessionFactory.getCache(). Finally, it
	 * calls the getMariadbConnection() method to obtain a reference to a MariaDB
	 * connection, which is assigned to the mariadbConnection field. Note that if
	 * the JDBC driver being used is not MariaDB, then mariadbConnection will be
	 * null.
	 *
	 * @throws BrokerException Signals that a broker exception has occurred.
	 */
	...
	/**
	 * This method takes a Session object as a parameter and returns a MariaDB jdbc
	 * Connection object. The method first checks if the flag GET_MARIADB_CONNECTION
	 * is set to true, and if so, it checks if the Session object is an instance of
	 * SessionImpl and if its physical connection is a NewProxyConnection that wraps
	 * a MariaDB jdbc Connection. If all of these conditions are true, the method
	 * returns the unwrapped MariaDB jdbc Connection object. If any of the
	 * conditions are false or if an exception is thrown during the process, the
	 * method returns null.
	 *
	...
	/**
	 * This method is used to get the identifier of a given object in Hibernate. It
	 * takes two parameters - the object and a Hibernate Session.
	 *
	 * If the object is a Hibernate proxy, it returns the identifier of the
	 * underlying persistent object. If not, it checks whether the Session's
	 * Metamodel is an instance of MetamodelImplementor (for Hibernate version 5)
	 * and the Session is an instance of SessionImplementor. If so, it obtains the
	 * entity persister for the object's class name and gets its identifier using
	 * the sessionImplementor. If none of these conditions are met, it throws a
	 * HibernateException.
	 *
	 * The return value is a Serializable object representing the identifier of the
	 * given object. *
	 *
	...
	/**
	 * This method is setting the read-only mode and auto-commit mode of a MariaDB
	 * database connection. In load-balancing or replication mode, this allow
	 * spreading the connections across the nodes.It first checks if a MariaDB jdbc
	 * Connection object is available and then sets the read-only and auto-commit
	 * mode using the setReadOnly() and setAutoCommit() methods, respectively. If
	 * the DEBUG_POOL flag is set, it also updates a NodeActivity object with the
	 * current read/write activity of the connection. If there is any SQLException
	 * during the process, it logs an error message with the read-only mode value.
	 *
	 * @param read the new read only
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Remove objects from the the Hibernate second level cache regions of the
	 * specified Class with the specified primary keys or all objects if no primary
	 * keys are specified.
	 */
	@Override
	 ...
	 *
	 * Remove objects from the the Hibernate second level cache regions of the
	 * specified table name with the specified primary keys or all objects if no
	 ...
	 *
	 * Before executing the query, it sets the connection to read-only mode and sets
	 * the session's default read-only mode to true. Then, it executes the query
	 * using the doReturningWork method of the session, which allows the query to be
	 * executed on the underlying JDBC connection.
	 *
	 * If the query execution results in an exception, the method catches the
	 * exception, logs it, and throws a BrokerException.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * This method takes a SQL query as input and executes it against the database.
	 * It sets the session to be not read-only, then creates a StringBuilder
	 * instance to keep track of the number of rows affected by the query. It then
	 * executes the query by creating a Statement instance from the database
	 * connection and calling its executeUpdate method, which returns an integer
	 * representing the number of rows affected. Finally, it clears the session and
	 * returns the number of rows as an integer. If any errors occur during the
	 * execution of the query, it logs an error message and throws a SQLException. *
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * This is a method that retrieves a specific object from the database using its
	 * primary key. It takes an object as a parameter and returns an object of the
	 * same type.
	 *
	 * It first sets the session to read-only mode to prevent any modifications to
	 * the object. Then, it retrieves the primary key value of the input object
	 * using the getObjectIdentifier method. Finally, it uses the session.get method
	 * to retrieve the object with the specified primary key from the database. If
	 ...
	 * {@inheritDoc}
	 *
	 * This method returns an iterator for all instances of a given class (specified
	 * by the target parameter) that are currently persisted in the database. It
	 * sets the session to read-only mode and creates a CriteriaQuery object to
	 * build the query. Then, it creates a Query object from the CriteriaQuery and
	 * executes the query to get the list of results. Finally, it returns an
	 * iterator for the list of results. If there is any error, it logs the error
	 * and throws a HibernateException.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * This method returns an iterator over a list of database objects of a specific
	 * type, based on a given SQL query. The SQL query is passed as a string
	 * parameter, and the type of the objects to be retrieved is passed as a generic
	 * type parameter.
	 *
	 * Inside the method, it sets the session to be read-only and creates a native
	 * SQL query using the createNativeQuery() method of the session object, passing
	 * the SQL string and the target class. It sets the fetch size and cacheable
	 * properties of the query, and returns an iterator over the list of results
	 * obtained by executing the query using the list() method of the query object.
	 *
	 * If any HibernateException occurs, it logs an error message and throws the
	 * exception.
	 *
	 * @see ecmwf.common.database.Broker#getIterator(java.lang.Class,
	 *      java.lang.String)
	 ...
	 * {@inheritDoc}
	 *
	 * This method is retrieving the primary key values of a given DataBaseObject
	 * instance. It first gets the Class object of the object and then retrieves the
	 * ListIterator of the columns that make up the primary key of the database
	 * table associated with the object. It then iterates over the primary key
	 * columns and gets the corresponding field from the object using reflection.
	 * The value of each field is added to a List of keys, which is then converted
	 * to an array of String and returned.
	 *
	 * @see ecmwf.common.database.Broker#getPrimaryKeyValues(ecmwf.common.database.
	 *      DataBaseObject)
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * This method performs a save or update of the given object depending on the
	 * update parameter.
	 *
	...
	/**
	 * This method performs a CRUD (Create, Read, Update, Delete) operation on the
	 * underlying database object. It begins by setting the connection to read-write
	 * mode. It then begins a transaction and executes the specified operation
	 * (saveOrUpdate, save, update, or delete) on the specified database object. If
	 * an exception occurs during the transaction, the transaction is rolled back
	 * and the exception is rethrown. Finally, the Hibernate session is cleared.
	 *
	 ...
	 *
	 * The class has several fields, including a DecimalFormat object called
	 * "format" (used to format numbers with two decimal points), a "startup" long
	 * that stores the system time when the object was created, and two AtomicLong
	 * objects called "reads" and "writes" that are used to track the number of read
	 * and write operations performed on the node.
	 *
	 * The constructor for the NodeActivity class takes a "hostAddress" argument,
	 * which is used to identify the database node being tracked.
	 *
	 * The class also has a method called "getAvgPerSec" that takes an AtomicLong
	 * object called "count" and returns a string representation of the average
	 * number of operations per second for that count, based on the time elapsed
	 * since the object was created.
	 *
	 * The main method in the class is "update", which takes a boolean argument
	 * called "read" that indicates whether a read or write operation has been
	 * performed. The method updates the appropriate AtomicLong object and, if the
	 * number of operations is a multiple of a constant called "DEBUG_FREQUENCY", it
	 * logs a message to a logger called "_log" indicating the number of reads and
	 * writes, along with their respective average operations per second. The
