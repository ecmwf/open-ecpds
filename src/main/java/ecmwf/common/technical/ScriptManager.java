...
public final class ScriptManager implements AutoCloseable {
	...
	/**
	 * The Constant EXPOSED_CLASSES. Classes which are allowed to be used within the
	 * scripts.
	 */
	...
	/**
	 * Exec. If no language is specified in the header of the script then the
	 * default language is used (e.g. script=js:code or script=python:code).
	 *
	...
	/**
	 * Instantiates a new script manager. Uses the requested script engine with a
	 * limit.
	 *
	...
	/**
	 * Gets the value, execute it with the provided arguments if it is a function
	 * and then cast it to the desired class.
	 *
	 * @param <T>   the generic type
	 * @param clazz the clazz
	 * @param name  the name
	...
	/**
	 * Gets the cache. If the context does not exists then create it and add the
	 * exposed classes!
	 *
	 * @return the cache
	 * @throws ScriptException the script exception
	...
	/**
	 * Called whenever the underlying script is trying to access an external class.
	 * It the external class is not listed in the exposed classes then the
