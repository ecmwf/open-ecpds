...
public final class MoverServer extends StarterServer implements MoverInterface {
	...
	/**
	 * The Class PurgeThread. This Thread is used to purge the file system (e.g.
	 * remove expired directories from the repository).
	 */
	 ...
	 * This method can be called from the MasterManager if it is connected through
	 * RMI or from the TransferRepository if it is connected through the REST
	 * interface.
	 */
	 ...
	 *
	 * @param restProvider the new REST provider
	 */
	 ...
	 * @param httpMover      the http mover
	 * @param connectTimeout the connect timeout
	 *
	 * @return the REST interface
	 *
	 * @throws MoverException the mover exception
	 */
	 ...
	 *
	 * @param mqttInterface the new MQTT interface
	 */
	...
	/**
	 * Gets the ECproxy plugging listen address and port. This is used by the Master
	 * Server to update the "TransferServer" entry in the database when the Mover
	 * subscribe. This address and port are used when the Master Server send the
	 * address of the allocated Mover to the ecpds command.
	 *
	 ...
	 * @param transfer the transfer
	 *
	 * @throws ECtransException the ectrans exception
	 * @throws IOException      Signals that an I/O exception has occurred.
	 */
	 ...
	 * @param hostsForSource the hosts for source
	 *
	 * @return the data file
	 *
	 * @throws ECtransException            the ectrans exception
	 * @throws SourceNotAvailableException the source not available exception
	 * @throws IOException                 Signals that an I/O exception has
	 *                                     occurred.
	 */
	@Override
	...
	/**
	 * Run a command on the server and return the output (e.g. process a report for
	 * a Host from the TransferServer).
	 *
	 * @param c the c
	 *
	 * @return the string
	 ...
	 *
	 * @param dataFile      the data file
	 * @param hostForSource the host for source
	 *
	 * @return the data file
	 *
	 * @throws SourceNotAvailableException the source not available exception
	 ...
	 * @param localPosn      the local posn
	...
	public DataTransfer put(final Host[] hostsForSource, final DataTransfer transfer, final String targetName,
		...
		try {
			...
			try {
				...
				transfer.setComment("Trying from " + (masterManager != null ? "DataMover=" + getRoot()
						: "Proxy Host=" + transfer.getProxyHostName() + " (" + transfer.getProxyHost().getNickname()
								+ ")"));
				successful = true;
				return transfer;
			} finally {
	...
	/**
	 * Allow getting a ProxySocket to upload a file on a target host. This method
	 * should be used only when the file is to be uploaded from a different data
	 * mover than the current one.
	 *
	 * @param host       the host
	...
	/**
	 * Allow getting a ProxySocket to retrieve a file on a target host. This method
	 * should be used only when the file is to be retrieved from a different data
	 * mover than the current one.
	 *
	 * @param host           the host
	 * @param source         the source
	 * @param remotePosn     the remote posn
	 * @param removeOriginal the remove original
	 *
	 * @return the proxy socket
	 *
	 * @throws ECtransException the ectrans exception
	...
	/**
	 * Allow providing an output stream to retrieve a file. This method should only
	 * be used when it is called from the current data mover. The retrieval is done
	 * asynchronously. If the retrieval fail then the provided Output Stream is
	 * closed.
	 *
	...
	/**
	 * If the data mover is connected directly to its MasterServer then return a RMI
	 * interface to the Master, if the data mover is connected through a delegate
	 * data mover then return a REST interface, otherwise through an exception.
	 *
	...
	public final class FileDescriptor {
		 ...
		 * @param length         the length
		 * @param filter         the filter
		 * @param inputFilter    the input filter (if the source file is filtered)
		 *
		 * @throws SourceNotAvailableException the source not available exception
		 * @throws IOException                 Signals that an I/O exception has
		 *                                     occurred.
		 ...
		 *
		 * @param host the new selected source host
		 */
	...
	public static final class ECproxyCallback extends DefaultCallback {
		 ...
		 * @param ticket    the ticket
		 * @param data      the data
		 * @param closeable something to close in case of failure of the transmission
		 */
	...
	private final class MoverCallback implements ProgressHandler, ECtransCallback {
		 ...
		 *
		 * @param descriptor the new file descriptor
		 */
		...
		public void completed(final TransferModule module) {
			...
			final var from = masterManager != null ? "DataMover=" + getRoot()
					: "Proxy Host=" + _transfer.getProxyHostName() + " (" + _transfer.getProxyHost().getNickname()
							+ ")";
	...
	/**
	 * The Class StreamRepository. Allow bypassing the ECproxy when loading files
	 * from the current data-mover.
	 */
