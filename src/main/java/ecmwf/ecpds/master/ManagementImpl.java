...
final class ManagementImpl extends CallBackObject implements ManagementInterface {
    ...
    final class CleanThread extends ExecutorRunnable {
        ...
        public void process() throws DataBaseException, MasterException {
            cleanDataTransfer(action, transfer);
        }
    }
     ...
     *             the data base exception
     */
    private void cleanDataTransfer(final ECpdsAction action, final DataTransfer transfer)
            throws MasterException, DataBaseException {
        ...
        if (master.getDataTransfers(transfer.getDataFileId()).length == 0) {
            master.purgeDataFile(transfer.getDataFile(), byAndFrom);
        }
    }
    ...
    public String cleanDestination(final ECpdsSession session, final String destinationName, final long days)
        ...
        final var destination = master.getDataBase().getDestination(destinationName);
        if (destination == null) {
            throw new MasterException("Destination not found: " + destinationName);
        }
        final var action = master.startECpdsAction(session, "clean", destination);
        Exception exception = null;
        String comment = null;
        var count = 0;
        Iterator<DataTransfer> it = null;
        try {
            it = base.getDataTransfersByDestinationAndTargetOnDateIterator(destination.getName(), null,
                    new java.util.Date(0), new java.util.Date(System.currentTimeMillis() - Timer.ONE_DAY * days));
            while (it.hasNext()) {
                final var transfer = it.next();
            ...
            comment = count + " DataTransfer(s) removed";
            ...
            master.logECpdsAction(action, "Removing from DataMover(s) DataTransfer(s) older than " + days + " day(s)"
                    + (isNotEmpty(comment) ? " (" + comment + ")" : ""), exception);
        }
        return monitor.done(comment);
    }
    ...
    public void removeTransferServer(final ECpdsSession session, final TransferServer server)
        ...
        try {
            ...
            final var it = base.getDataTransfersByTransferServer(server);
            try {
                while (it.hasNext()) {
                    cleanDataTransfer(action, it.next());
                }
            } finally {
                if (it != null) {
                    it.remove();
    ...
    public boolean transferStatusUpdateAllowed(final long id, final String code) throws MasterException {
        ...
        return monitor.done(master.updateTransferStatus(id, code, false, null, null, true, false));
    }
    ...
    public boolean updateTransferStatus(final ECpdsSession session, final long id, final String code)
        ...
        try {
            result = master.updateTransferStatus(id, code, true, action.getWebUserId(),
                    "By WebUser=" + webUser.getId() + " (" + webUser.getName() + ") from the web interface", true,
                    true);
            if (result) {
                master.reloadDestination(transfer);
            }
