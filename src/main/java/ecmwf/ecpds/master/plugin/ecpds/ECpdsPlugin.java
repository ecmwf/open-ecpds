...
public final class ECpdsPlugin extends SimplePlugin implements ProgressInterface {
	 ...
	 *
	 * Utility method to check the size of a parameter (e.g. identity, original,
	 * source and target).
	 *
	 * @param key   the key
	 ...
	 *
	 * If the product date was not set then let's use the timeFile if we have it or
	 * then let's use the current time!
	 *
				...
				if (forceStop) {
					...
					if (StatusFactory.SCHE.equals(thisStatus)) {
						...
						transfer.setComment("Scheduled for " + (transfer.getAsap() ? "as soon as possible"
								: "no sooner than " + Format.formatTime(DEFAULT_DATE_FORMAT,
	 ...
	 *
	 * @param ecproxyList the ecproxy list
	 *
	 * @return the string
	 */
