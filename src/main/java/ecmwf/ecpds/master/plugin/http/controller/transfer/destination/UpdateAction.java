 ...
 *
 * Update a Destination. Only its proper fields
 *
...
public class UpdateAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        if (c.size() == 1) {
            ...
            final var destination = DestinationHome.findByPrimaryKey(c.iterator().next().toString());
            ((DestinationActionForm) form).populateDestination(destination);
            destination.save(user);
            request.setAttribute("destination", destination);
            return mapping.findForward("success");
        } else {
            throw new ECMWFActionFormException("Expected 1 parameter. Got " + c.size());
        }
    }
}
