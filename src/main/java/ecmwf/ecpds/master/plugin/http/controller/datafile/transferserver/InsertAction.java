...
public class InsertAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        final var daf = (TransferServerActionForm) form;
        final var name = daf.getName();
        if (!Format.isValidId(name, "-_")) {
            throw new ECMWFActionFormException(
                    "Please choose a valid non-empty Transfer Server name (only letters, digits, '_' and '-' are allowed): \""
                            + name + "\"");
        }
        final var d = TransferServerHome.create();
        daf.populateTransferServer(d);
        d.insert(user);
