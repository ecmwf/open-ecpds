...
public class UpdateAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        if (!this.isCancelled(request)) {
            final var methodName = ECMWFActionForm.getPathParameter(mapping, request, 0);
            final var haf = (TransferMethodActionForm) form;
            final var h = TransferMethodHome.findByPrimaryKey(methodName);
            haf.populateTransferMethod(h);
            h.save(user);
