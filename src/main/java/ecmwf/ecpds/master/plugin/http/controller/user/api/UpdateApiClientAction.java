...
public class UpdateApiClientAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        try {
            ...
            client.setComment(actionForm.getComment() != null ? actionForm.getComment().trim() : "");
            client.setActive(Boolean.parseBoolean(actionForm.getActive()));
            final var countryIso = actionForm.getCountryIso();
            client.setCountryIso(countryIso != null && !countryIso.isBlank() ? countryIso.trim() : null);
            db.update(Util.getECpdsSessionFromObject(user), client);
        } catch (final Exception e) {
            throw new ECMWFException("Error updating API client: " + e.getMessage(), e);
        }
