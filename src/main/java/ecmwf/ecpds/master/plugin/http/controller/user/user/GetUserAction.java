 ...
 */
public class GetUserAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        if (!parameters.isEmpty()) {
            request.setAttribute("user", UserHome.findByPrimaryKey(parameters.get(0).toString()));
            return mapping.findForward("success");
        }
        final var search = request.getParameter("search");
        Collection<ModelBean> users = UserHome.findAll();
    ...
    public boolean match(final ModelBean b, String what) {
        ...
        return u.getCommonName().toLowerCase().contains(what) || u.getName().toLowerCase().contains(what);
    }
}
