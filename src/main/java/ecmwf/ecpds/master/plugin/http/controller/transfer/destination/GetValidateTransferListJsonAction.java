 ...
 *
 * Returns a DataTables-compatible JSON payload for the validate (session-selected) transfer list,
 * enabling server-side pagination of the transfers the user has chosen for batch operations.
 *
...
/**
 * The Class GetValidateTransferListJsonAction.
 *
 * Handles AJAX DataTables server-side requests for the validate (selection) transfer table. Reads the session-selected
...
public class GetValidateTransferListJsonAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        var length = parseSafeInt(request.getParameter("length"), 25);
        if (length < 1) {
            length = 25;
        }
        ...
        try {
            ...
            if (daf != null && destinationName != null && !destinationName.isEmpty()) {
        ...
        // All visible transfers default to batch-selected
        ...
        try {
            ...
            writeError(response, draw, "Error building validate transfer list: " + e.getMessage());
        }
        return null;
    }
    ...
    private static String buildValidateSelectHtml(final DataTransfer dt, final String destName) {
        if (dt.getExpired()) {
            return "";
        }
        final var id = escapeHtml(dt.getId());
        final var eDest = escapeHtml(destName);
        return "<span class=\"d-flex gap-2 align-items-center\">" + "<span class=\"batch-select\" data-transfer-id=\""
                + id + "\"" + " onclick=\"select(this,'" + id
                + "')\" style=\"cursor:pointer\" title=\"Toggle batch selection\">"
                + "<i class=\"bi bi-star fs-6\"></i></span>" + "<a href=\"/do/transfer/destination/operations/" + eDest
                + "/unselectTransfer/" + id
                + "\" title=\"Remove from selection\"><i class=\"bi bi-x-circle text-danger\"></i></a>" + "</span>";
    }
