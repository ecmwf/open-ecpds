 ...
 *
 * Deletes all outstanding (bad) transfers for a single destination without
 * loading the full set into Java memory. Transfers are processed in batches;
 * after each batch is deleted those records are gone so the next iteration
 * always fetches the new "first page" of remaining bad transfers.
 *
...
/**
 * The Class DeleteDestinationBadTransfersAction.
 */
public class DeleteDestinationBadTransfersAction extends PDSAction {

    /** The Constant log. */
    private static final Logger log = LogManager.getLogger(DeleteDestinationBadTransfersAction.class);
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        if ("true".equals(request.getParameter("delete"))) {
            ...
            // Process in batches. After delete, those records are gone so we always fetch
            // from offset 0 — the next batch of still-bad transfers becomes the new first page.
        ...
        try {
            ...
            log.warn("Redirect failed after delete for destination '{}'", destinationName, e);
        }
        return null;
    }
}
