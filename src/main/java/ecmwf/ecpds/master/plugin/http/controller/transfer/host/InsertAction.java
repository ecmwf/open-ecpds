...
public class InsertAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        try {
            ...
            final var nickName = haf.getNickName();
            if (isNotEmpty(haf.getNickName())) {
                final var h = HostHome.create();
                haf.setNickName(nickName.trim());
                haf.populateHost(h);
                h.insert(user);
            } else {
                throw new Exception("Error while trying to CREATE a new Host (nickname is empty)");
            }
