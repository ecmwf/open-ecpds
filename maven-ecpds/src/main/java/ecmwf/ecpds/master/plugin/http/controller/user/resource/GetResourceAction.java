...
public class GetResourceAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        if (!parameters.isEmpty()) {
            request.setAttribute("resource", ResourceHome.findByURI(parameters.get(0).toString()));
            return mapping.findForward("success");
        }
        final var search = request.getParameter("search");
        Collection<ModelBean> resources = ResourceHome.findAll();
    ...
    public boolean match(final ModelBean b, String what) {
        final var r = (Resource) b;
        what = what.toLowerCase();
        return r.getPath().toLowerCase().contains(what) || r.getHost().toLowerCase().contains(what);
    }
}
