...
public class InsertAction extends PDSAction {
    ...
    public ActionForward safeAuthorizedPerform(final ActionMapping mapping, final ActionForm form,
        ...
        try {
            final var inu = (IncomingPolicyActionForm) form;
            final var incomingPolicy = IncomingPolicyHome.create();
            final var id = inu.getId();
            if (Format.isValidId(id, "_.")) {
                inu.setId(id.trim());
                inu.populatePolicy(incomingPolicy);
                incomingPolicy.insert(user);
                return mapping.findForward("success");
            } else {
                throw new Exception(
                        "Please choose a valid non-empty Data Policy name (only letters, digits, '_' and '.' are allowed): \""
                                + id + "\"");
            }
