...
## Compute the statistics (make sure that we have not yet computed the statistics before to avoid duplicates)
INSERT INTO STATISTICS SELECT
  UNIX_TIMESTAMP(DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d 00:00:00"))*1000 AS DATE,
  TRANSFER_SERVER.TRG_NAME,
  DATA_TRANSFER.STA_CODE,
  DES_TYPE,
  count(distinct(DATA_TRANSFER.DES_NAME)) AS DESTINATION,
  SUM(DAT_SIZE) AS SIZE
FROM
  DATA_TRANSFER,
  TRANSFER_SERVER,
  DESTINATION
WHERE
  (SELECT COUNT(*) FROM STATISTICS WHERE DATE = UNIX_TIMESTAMP(DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d 00:00:00"))*1000) = 0
  AND FROM_UNIXTIME(DAT_QUEUE_TIME/1000,'%Y-%m-%d') = DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d")
  AND DESTINATION.DES_NAME = DATA_TRANSFER.DES_NAME
  AND DATA_TRANSFER.TRS_NAME = TRANSFER_SERVER.TRS_NAME
GROUP BY
  DES_TYPE, 
  STA_CODE,
  TRG_NAME;
...
INSERT INTO DISK_USAGE SELECT
  TRANSFER_GROUP.TRG_NAME AS TRG_NAME,
  UNIX_TIMESTAMP(DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d 00:00:00"))*1000 AS DIU_DATE,
  COUNT(*) AS DIU_FILES,
  SUM(IF(TRG_REPLICATE,TRG_MIN_REPLICATION_COUNT*DAF_SIZE,DAF_SIZE)+IF(TRG_FILTER AND DAF_FILTER_SIZE > 0,TRG_MIN_FILTERING_COUNT*DAF_FILTER_SIZE,0)) AS DIU_BYTES
FROM
  DATA_FILE,
  TRANSFER_GROUP
WHERE
  (SELECT COUNT(*) FROM DISK_USAGE WHERE DIU_DATE = UNIX_TIMESTAMP(DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d 00:00:00"))*1000) = 0
  AND TRANSFER_GROUP.TRG_NAME = DATA_FILE.TRG_NAME
  AND DAF_DOWNLOADED
  AND NOT DAF_REMOVED
GROUP BY
  TRANSFER_GROUP.TRG_NAME;

## Compute the q2diss statistics for delays
INSERT INTO Q2DISS_REPORT SELECT
  CONCAT(DAF_META_TIME,'-',DAF_META_STREAM) AS PRODUCT,
  DAF_TIME_STEP,
  DAF_META_TYPE,
  DESTINATION.DES_NAME AS DES_NAME,
  MAX(DAF_GET_TIME) AS LAST_RETRIEVED,
  DAT_SCHEDULED_TIME AS SCHEDULE,
  FLOOR((DAT_SCHEDULED_TIME-MAX(DAF_GET_TIME))/1000/60) AS MINUTES,
...
FROM
  ...
  DESTINATION
WHERE
  DESTINATION.DES_NAME = DATA_TRANSFER.DES_NAME
  AND DATA_FILE.DAF_ID = DATA_TRANSFER.DAF_ID  
  AND NOT DATA_TRANSFER.STA_CODE = 'HOLD'
  AND NOT DAT_DELETED
  AND NOT DESTINATION.STA_CODE = 'STOP'
  AND DESTINATION.DES_MONITOR
  AND FROM_UNIXTIME(DAF_GET_TIME/1000,'%Y-%m-%d') = DATE_FORMAT(DATE(NOW()-INTERVAL 1 DAY), "%Y-%m-%d")
  AND DAF_CALLER LIKE '%/emos/%/q2diss%'
GROUP BY
  DAF_META_TIME,
  DAF_META_STREAM,
  DAF_TIME_STEP,  
  ...
  DES_NAME
HAVING
  FLOOR((DAT_SCHEDULED_TIME-MAX(DAF_GET_TIME))/1000/60) < -10
ORDER BY
  PRODUCT,
  DES_NAME,
  SCHEDULE;
